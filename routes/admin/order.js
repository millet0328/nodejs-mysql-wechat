...
router.get('/list', async function (req, res) {
    ...
    // 查询订单商品信息
    let goods_sql = `SELECT g.id, g.name, g.img_md, og.goods_num, og.goods_price, og.order_id FROM ( SELECT id FROM orders ORDER BY create_time DESC LIMIT ? OFFSET ? ) AS o JOIN order_goods og ON og.order_id = o.id JOIN goods g ON g.id = og.goods_id`;
    // 循环遍历给订单数组添加订单商品信息
