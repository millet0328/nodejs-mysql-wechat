...
router.post('/', async (req, res) => {
    try {
        ...
        if (affectedRows === 0) {
            res.json({ status: false, msg: "创建role失败！" });
            return;
        }
        res.json({
            status: true,
...
router.post('/menu', async (req, res) => {
    ...
    try {
        ...
        let [exist_menus] = await connection.query(select_sql, [id]);
        exist_menus = exist_menus.map((item) => item.menu_id);
        //计算两个数组的差集
        let rest_exist_menus = exist_menus.filter((item) => !insert_menus.includes(item));
        let rest_insert_menus = insert_menus.filter((item) => !exist_menus.includes(item));
        //转化数组格式
...
router.delete('/menu/:role_id', async (req, res) => {
    try {
